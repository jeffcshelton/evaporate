...
impl AddressBook {
	...
	fn get_phone_number(&self, name: &str) -> Result<String> {
		...
		let person_id: i32 = person_rows.next()?.unwrap().get(0)?; // TODO: Internal, but remove .unwrap()
		...
		let phone_number: String = number_rows.next()?.unwrap().get(0)?; // TODO: Internal, but remove .unwrap()
